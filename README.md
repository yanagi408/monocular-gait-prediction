...
支持脚入力を加えることで、短期予測における下肢形状のずれが小さくなった。発表資料でも、支持脚入力ありの条件で正解座標との重なりが改善していることを示している。:contentReference\[oaicite:0]{index=0}
...
予測長を 10 / 20 / 30 フレームと長くするほど、正解座標との重なりが悪化し、長期予測で誤差が蓄積しやすい傾向が確認された。発表資料でも、予測長が長いほど下肢形状の予測精度が低下すると整理している。:contentReference\[oaicite:1]{index=1}
...
支持脚入力は、支持脚そのものの推定だけでなく、特に支持脚切替えタイミングの推定精度改善に有効だった。発表資料では、支持脚入力ありの条件で切替え推定精度が大きく向上したと示している。:contentReference\[oaicite:2]{index=2}
...
複数人物条件・予測長10フレームでは、支持脚入力により `pos MAE` は `0.1508 → 0.0936`、`Z MAE` は `0.3557 → 0.2267`、`支持脚F1` は `0.5323 → 0.5964` に改善した。:contentReference\[oaicite:3]{index=3}  

一方で、予測長30フレームでは `pos MAE = 0.3156`、`Z MAE = 0.7409`、`cos類似度 = 0.8688` となり、長期予測では精度低下が大きくなることを確認した。:contentReference\[oaicite:4]{index=4}
